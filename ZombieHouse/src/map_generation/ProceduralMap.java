...
public class ProceduralMap
{
  ...
  public static Tile[][] generateMap(int rows, int cols, int difficulty){
    ...
    getBorderWallNeighborRegions(map);
  ...
  private static ArrayList<ArrayList<Rectangle>> generateRegions(Rectangle[][] basicMap){
    Random random = new Random(0); // Added a seed so that the results are repeatable.
  ...
  private static void combineAdjacentHallways(ArrayList<Rectangle> region){
   ...
   * 
   * @param regions arrayLists contains arrayLists of rectangles
   */
  private static void generatePaths(ArrayList<ArrayList<Rectangle>> regions){
    Random random = new Random();
      ...
      for (Rectangle r: regions.get(i)){
        ...
        for (Rectangle path : r.paths){
          if (path.x == r.x + r.width){ //on right of r
            ...
            map[y][path.x].setRegion(r.region);

            map[y+1][path.x] = new Tile(r.region, y+1,path.x, !r.isRoom);
            map[y+1][path.x].setRegion(r.region);
          }
          if (path.y == r.y + r.height){//on bottom of r
            ...
            map[path.y][x].setRegion(r.region);

            map[path.y][x+1] = new Tile(r.region,path.y,x+1, !r.isRoom);
            map[path.y][x+1].setRegion(r.region);
          }
        }
      }
    }

    //top left corner wall (it is not visible)
    map[0][0] = new Tile(0,0,0,false);
    map[0][0].setRegion(1);

    //left border wall
    for (int i = 1; i < map.length; i++){
      map[i][0] = new Tile(0,i,0, false);
      map[i][0].setRegion(map[i][1].getRegion());
    }

    //top border wall
    for (int i = 1; i < map[0].length; i++){
      map[0][i] = new Tile(0,0,i, false);
      map[0][i].setRegion(map[1][i].getRegion());
    }
   ...
   * finds the adjacent (neighboring) region to all walls
   * @param map
   */
  private static void getBorderWallNeighborRegions(Tile[][] map) {
    for (int row = 0; row < map.length; row++) {
      for (int col = 0; col < map[0].length; col++) {
        if (map[row][col].type == Tile.TileType.wall) {
          if (checkNeighbor(map, row, col, 0, 1) || checkNeighbor(map, row, col, 0, -1) || checkNeighbor(map, row, col, 1, 0)
                  || checkNeighbor(map, row, col, -1, 0)) {
            map[row][col].isBorder = true;
          }
        }
      }
    }
   ...
   *@author Hector Carrillo
   * @param map
   * @param row of tile in question
   * @param rowOffset direction to look in
   * @param col of tile in question
   * @param colOffset direction to look in
   * @return
   */
  private static boolean checkNeighbor(Tile[][] map, int row, int col, int rowOffset, int colOffset)
  {
    // the neighbor it is checking is in bounds
    if(row+rowOffset < map.length && row+rowOffset >= 0 &&
            col+colOffset < map[0].length && col+colOffset >= 0)
    {
      // the region of the nighboring tile is not the same so it is a border
      if(map[row][col].getRegion() != map[row+rowOffset][col+colOffset].getRegion())
      {
        map[row][col].adjacentRegionRowOffset = rowOffset;
        map[row][col].adjacentRegionColOffset = colOffset;
        map[row][col].adjacentRegion = map[row+rowOffset][col+colOffset].getRegion();
        return true;
      }
    }
    return false;
   ...
   */
    ...
    private ArrayList<Rectangle> neighbors = new ArrayList<>(); //represents adjacency
    private ArrayList<Rectangle> paths = new ArrayList<>(); //represents openings
     ...
     */
    private boolean equals(Rectangle r){
     ...
     */
    private Rectangle[] getSubRectangles(int orientation, double percentage){
     ...
     */
    private Rectangle splitOffHallway(int orientation){
      Rectangle hallway;
