...
public class Pathfinding
{
    ...
    public static Map<PathNode,PathNode> getPath(PathNode[][] board, PathNode start_position, PathNode end_position)
    {
        ...
        while( !frontier.isEmpty() )
        {
            ...
            // Diagonals. We don't want to clip corners, so only allow diagonals if there's space around.
...
//            if( board[current.x+1][current.y-1] != null ){ neighbors.add( board[current.x+1][current.y-1]); }
            ...
            // Diagonals: We don't want to clip corners, so only allow diagonals if there's space around.
            if( board[current.x-1][current.y+1] != null
                    && board[current.x-1][current.y] != null
                    && board[current.x][current.y+1] != null
                    ){ neighbors.add( board[current.x-1][current.y+1]); }
